...
sleep 2
screen -p 0 -S $NAME -X stuff $"set stream opts 0:pid=1000,vbv-maxrate=500,bitrate=400,keyint=30,bframes=1,threads=6,format=avc,profile=high,level=5.1,aspect-ratio=16:9,intra-refresh=1\012"
screen -p 0 -S $NAME -X stuff $"set stream opts 1:pid=1001,bitrate=64,format=mp2,aac-profile=,aac-encap=\012"
screen -p 0 -S $NAME -X stuff $"set muxer opts ts-type=dvb,ts-muxrate=700000\012"
screen -p 0 -S $NAME -X stuff $"set output udp\012"
screen -p 0 -S $NAME -X stuff $"set outputs 1\012"
screen -p 0 -S $NAME -X stuff $"set output opts 0:target=udp://204.13.45.187:3000\012"
